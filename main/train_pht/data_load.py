...
class LoadWaveform(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        wf_tensor = torch.from_numpy(wf).float().permute(1, 0)[:, :3000]
       ...
       # det[det_index] = 1
        ...
        s = generate_label(3000, s_sample, "gaussian", 30)

        p = torch.from_numpy(p).float()
        s = torch.from_numpy(s).float()
        label = torch.stack((p, s, 1-(p+s)), 0)
        #label = torch.stack((s, det, 1-s),0)
