...
def main(args):
    ...
    ## load model and data
    #weight = torch.load('results/039-PhaseNet/checkpoints/0030000.pt')
    #model = EQTransformer(original_compatible=args.original_compatible, lstm_blocks=args.lstm_blocks).to(device)
    model = VariableLengthPhaseNet(in_samples=1500, sampling_rate=50).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for wf, labels in tqdm(train_loader):
            wf = wf.to(device)
            output = model(wf)
            output_p, output_s, output_d = output[:, 0, :], output[:, 1, :], output[:, 2, :]
